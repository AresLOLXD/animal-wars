...
import { SCALE_FACTOR, TIME_CHECK_KEY_PRESSED, TIME_SIMON_SAYS, TIME_SIMON_SAYS_PLAYER, TIME_TEXT_PLAYER, calculateDimensions, calculateLogarithmTime } from "@phaser/Util"
import { Scene, Math as MathP, Input } from "phaser";
...
export class SimonSays extends Scene {
    ...
    preload() {
        ...
        this.player_one = new Player(this, this.player_one_positionX, this.half_height, "capibara")
        this.player_one.setFlipX(true);
        this.player_one.setScale(SCALE_FACTOR);

        this.player_two = new Player(this, this.player_two_positionX, this.half_height, "husky")
        this.player_two.setScale(SCALE_FACTOR);

        this.simon = new Simon(this, this.half_width, this.half_height - 200, "panda");
    ...
    evaluateTurn() {
        ...
        this.time.delayedCall(1000, () => {
            const player_one_correct = this.compareAnswers(this.player_one_values, true);
            const player_two_correct = this.compareAnswers(this.player_two_values, true);

            if (player_one_correct && player_two_correct) {
                this.drawText("Empate", this.half_width, this.half_height, 1000);
            } else if (player_one_correct) {
                this.drawText("Gano el jugador 1", this.half_width, this.half_height, 1000);
            } else if (player_two_correct) {
                this.drawText("Gano el jugador 2", this.half_width, this.half_height, 1000);
            } else {
                this.drawText("Perdieron los dos", this.half_width, this.half_height, 1000);
            }
            this.time.delayedCall(1000, () => {
                this.state = States.SimonTurn;
    ...
    simonTurn() {
        ...
        const simon_time = calculateLogarithmTime(this.simonSaysValues.length, TIME_SIMON_SAYS);
        const total_time = (simon_time * this.simonSaysValues.length) + (800 * this.simonSaysValues.length);
        ...
        for (let i = 0; i < this.simonSaysValues.length; i++) {
            ...
            this.time.addEvent({
                ...
                callback: () => {
                    this.drawValue(letter, this.half_width, this.half_height, simon_time - 200);
        ...
        this.time.delayedCall(total_time + 200, () => {
            ...
            const player_time = calculateLogarithmTime(this.simonSaysValues.length, TIME_SIMON_SAYS_PLAYER) * this.simonSaysValues.length;
            this.time.delayedCall(player_time, () => {
                ...
                this.drawText("Se acabo el tiempo", this.half_width, this.half_height, 1000);
            });
        });
    ...
    getAnswer(value: Values, isPlayerOne: boolean) {
        if (isPlayerOne && this.player_one_values.length < this.simonSaysValues.length) {
        ...
        } else if (!isPlayerOne && this.player_two_values.length < this.simonSaysValues.length) {
    ...
    playersTurn() {
        if (this.listen_player_one_keys) {
            if (this.input.keyboard!.checkDown(this.keys_player_one!.up, TIME_CHECK_KEY_PRESSED)) {
                this.drawText("Arriba", this.player_one_positionX, this.player_one!.y - 100, TIME_TEXT_PLAYER);
                this.getAnswer(Values.UP, true);
            } else if (this.input.keyboard!.checkDown(this.keys_player_one!.down, TIME_CHECK_KEY_PRESSED)) {
                this.drawText("Abajo", this.player_one_positionX, this.player_one!.y - 100, TIME_TEXT_PLAYER);
                this.getAnswer(Values.DOWN, true);
            } else if (this.input.keyboard!.checkDown(this.keys_player_one!.left, TIME_CHECK_KEY_PRESSED)) {
                this.drawText("Izquierda", this.player_one_positionX, this.player_one!.y - 100, TIME_TEXT_PLAYER);
                this.getAnswer(Values.LEFT, true);
            }
            else if (this.input.keyboard!.checkDown(this.keys_player_one!.right, TIME_CHECK_KEY_PRESSED)) {
                this.drawText("Derecha", this.player_one_positionX, this.player_one!.y - 100, TIME_TEXT_PLAYER);
                this.getAnswer(Values.RIGHT, true);
            }
        }
        if (this.listen_player_two_keys) {
            if (this.input.keyboard!.checkDown(this.keys_player_two!.up, TIME_CHECK_KEY_PRESSED)) {
                this.drawText("Arriba", this.player_two_positionX, this.player_two!.y - 100, TIME_TEXT_PLAYER);
                this.getAnswer(Values.UP, false);
            } else if (this.input.keyboard!.checkDown(this.keys_player_two!.down, TIME_CHECK_KEY_PRESSED)) {
                this.drawText("Abajo", this.player_two_positionX, this.player_two!.y - 100, TIME_TEXT_PLAYER);
                this.getAnswer(Values.DOWN, false);
            } else if (this.input.keyboard!.checkDown(this.keys_player_two!.left, TIME_CHECK_KEY_PRESSED)) {
                this.drawText("Izquierda", this.player_two_positionX, this.player_two!.y - 100, TIME_TEXT_PLAYER);
                this.getAnswer(Values.LEFT, false);
            } else if (this.input.keyboard!.checkDown(this.keys_player_two!.right, TIME_CHECK_KEY_PRESSED)) {
                this.drawText("Derecha", this.player_two_positionX, this.player_two!.y - 100, TIME_TEXT_PLAYER);
